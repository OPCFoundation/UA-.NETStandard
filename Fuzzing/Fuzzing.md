...
[libFuzzer](https://llvm.org/docs/LibFuzzer.html). Each fuzz area lives under `Fuzzing/<Area>/`
and shares a generic NUnit replay harness, a `Fuzz.Tools` corpus generator / playback driver,
and the `common/Fuzz` afl/libFuzzer host.

## Areas

| Area | Surface | Status |
|------|---------|--------|
| `Encoders/` | BinaryDecoder / JsonDecoder / XmlDecoder + idempotent round-trip, individual built-in type readers (NodeId, ExpandedNodeId, Variant, ExtensionObject, DataValue, DiagnosticInfo, QualifiedName, LocalizedText) and parser entry points (`NodeId.Parse`, `ExpandedNodeId.Parse`, `RelativePathFormatter.Parse`, `QualifiedName.Parse`, `NumericRange.Parse`, `Uuid` round-trip) | shipped |
| `Certificates/` | `X509CRL` decode + extensions (`X509SubjectAltNameExtension`, `X509AuthorityKeyIdentifierExtension`, `X509CrlNumberExtension`), `PEMReader` cert/key import, `Pkcs10CertificationRequest`, low-level `AsnUtils` helpers | shipped |
| `Network/` | OPC UA UA-SC / TCP framing via `Opc.Ua.Bindings.Pcap` (`OpcUaFrameParser`, `TcpStreamReassembler`, `OfflineSecureChannel.ReadChunk`, `ServiceCallReassembler`, `MockServerReplay`/`MockClientReplay` stateful drivers) and the `Opc.Ua.Core` UA-SC parse seam (`TcpMessageParsers.TryParseChunkHeader` / `ReadHelloMessage` / `ReadAcknowledgeMessage` / `ReadErrorMessage` / `ReadReverseHelloMessage` / `ReadAsymmetricMessageHeader`) | shipped |
...
Fuzzing/
  ...
  common/
    Fuzz/                              # SharpFuzz host (Program.cs + FuzzMethods.cs)
    Fuzz.Tests/                        # shared NUnit harness
      FuzzTargetTestsBase.cs           #   generic reflection-driven base fixture
      FuzzTargetFunction.cs            #   reflection wrapper around a static target
      TestAssetUtils.cs                #   IAsset / AssetCollection / TestUtils
      TestcaseAsset.cs                 #   asset loaded from disk
    Fuzz.Tools/                        # shared Program/Playback/Logging/Testcases base
  dictionaries/
    binary.dict json.dict xml.dict     # encoder dictionaries
    asn1.dict                          # ASN.1 / DER / PEM dictionary (Certificates)
    nodeid.dict                        # NodeId / RelativePath / NumericRange parser dictionary
    uasc.dict tcp.dict                 # OPC UA UA-SC + raw TCP framing (Network)
  scripts/
    fuzz-afl.ps1 fuzz-libfuzzer.ps1    # area-agnostic per-target runners
    fuzz-menu.ps1                      # dynamic target menu (lists FuzzableCode statics)
    install.sh                         # afl + sharpfuzz setup
  Certificates/  Fuzz | Fuzz.Tests | Fuzz.Tools
  Encoders/      Fuzz | Fuzz.Tests | Fuzz.Tools
  Network/       Fuzz | Fuzz.Tests | Fuzz.Tools
```
...
* the generic NUnit harness (`common/Fuzz.Tests/FuzzTargetTestsBase.cs`) at test time —
  every target is replayed against the area's `Testcases.*/` corpus plus crash/timeout
  assets, with no per-area test code required.
...
2. **Copy `Fuzzing/Encoders/`** to `Fuzzing/<NewArea>/`. Rename the three csproj files
   (`<NewArea>.Fuzz.csproj`, `Opc.Ua.<NewArea>.Fuzz.Tests.csproj`, `<NewArea>.Fuzz.Tools.csproj`)
   and update assembly / root namespaces.
3. **Replace `FuzzableCode.*.cs`** with your area's targets. Naming convention:
   ...
   the fuzzer finds.
4. **Subclass the harness.** `Fuzz.Tests/<Area>Tests.cs` inherits from
   ...
   `CreateFuzzTargetFunctions(typeof(FuzzableCode))`.
5. **Generate seeds.** Add `<Area>.Testcases.cs` to `Fuzz.Tools/` that builds valid sample
   inputs through the real producer code and writes them under `Fuzz/Testcases.<Bucket>/`.
   Each `Testcases.*/` subdirectory becomes a logical seed bucket and is auto-discovered
   by `TestAssetUtils.DiscoverTestcaseEncoderSuffixes`.
6. **Add a dictionary.** Put `<Area>.dict` under `Fuzzing/dictionaries/` with common
   tokens / magic bytes / length sentinels — this dramatically improves libFuzzer
   coverage progression.
7. **Wire into `UA.slnx`.** Add the three new projects under
   `/Fuzzing/<Area>/`, the dictionary under `/Fuzzing/Dictionaries/`, and any seed-corpus
   loose files you want visible in the IDE.
8. **Update this `Fuzzing.md`.** Add a row to the area table.

The Azure pipelines (`test.yml` / `testcc.yml`) automatically run every `*.Fuzz.Tests`
project under the `[Category("Fuzzing")]` filter.
...
* **Phase 4b (internal Core seam).** Adds `internal static class TcpMessageParsers` in
  `Stack/Opc.Ua.Core/Stack/Tcp/TcpMessageParsers.cs` covering the pre-crypto,
  pre-auth chunk surface the pcap binding does not expose: `TryParseChunkHeader`,
  `ReadHelloMessage`, `ReadAcknowledgeMessage`, `ReadErrorMessage`,
  `ReadReverseHelloMessage`, `ReadAsymmetricMessageHeader`. Surfaced to the fuzz area via
  `<InternalsVisibleTo Include="Network.Fuzz" />` (and the matching `*.Tools` / `*.Tests`
  assemblies so the linked partials compile from every host).
...
* UA-SC chunks → `Testcases.Chunks/`, `Testcases.Tcp.Hello/`, `Testcases.Tcp.Ack/`,
  `Testcases.Tcp.Err/`, `Testcases.Tcp.Rhe/`, `Testcases.Tcp.AsymHdr/`
* paired `ChannelKeyMaterial` JSON → `Testcases.Keys/`
...
powershell -File Fuzzing/scripts/fuzz-menu.ps1 `
    -AssemblyPath Fuzzing/Network/Fuzz/bin/Debug/net10.0/Network.Fuzz.dll
...
matching one of the listed names; the area-local `libfuzz.bat` / `aflfuzz.sh` wrappers
chain through to those.

```powershell
cd Fuzzing/Encoders
./libfuzz.bat                           # interactive menu (Windows)
./libfuzz.sh                            # interactive menu (Linux)
./aflfuzz.sh                            # afl-fuzz on Linux only
```
...
```bash
dotnet run --project Fuzzing/Network/Fuzz.Tools -- --playback --stacktrace
```
...
```bash
dotnet run --project Fuzzing/Network/Fuzz.Tools -- --testcases
```
...
byte-stable artifacts the harness's `Testcases.*/` discovery picks up automatically.
