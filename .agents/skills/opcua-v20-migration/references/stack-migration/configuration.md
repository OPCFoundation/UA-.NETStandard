...
- `ApplicationConfiguration.LoadWithNoValidation` uses `XmlParser`/`IEncodeable.Decode()`. Existing XML config files should remain loadable.
  Applications moving to the dependency-injection surface can pass such a file directly to `services.AddOpcUa().AddServer("MyServer.Config.xml")` or `.AddClient("MyClient.Config.xml")` and keep every setting from it — see [Migrating with an existing configuration XML file](https://github.com/OPCFoundation/UA-.NETStandard/blob/master/docs/DependencyInjection.md#migrating-with-an-existing-configuration-xml-file) and [Client: using an existing configuration XML file](https://github.com/OPCFoundation/UA-.NETStandard/blob/master/docs/DependencyInjection.md#client-using-an-existing-configuration-xml-file).
...
See [Dependency Injection](https://github.com/OPCFoundation/UA-.NETStandard/blob/master/docs/DependencyInjection.md#server-feature),
[Alias Names](https://github.com/OPCFoundation/UA-.NETStandard/blob/master/docs/AliasNames.md), and
[Reverse Connect](https://github.com/OPCFoundation/UA-.NETStandard/blob/master/docs/ReverseConnect.md#server-side-dependency-injection)
...
[NodeManagers.md § Sampling interval revision](https://github.com/OPCFoundation/UA-.NETStandard/blob/master/docs/NodeManagers.md#sampling-interval-revision)
for the full rule.
