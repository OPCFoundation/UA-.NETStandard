...
try {
    ...
    $operation = switch ($Scenario) {
        { $_ -in @('pilot-incomplete', 'required-stable', 'required-four-part-version', 'required-preview') } {
            'Preflight'; break
        }
    ...
    $expected = if ($Scenario -in @(
        ...
        elseif ($Scenario -in @('wrong-digest', 'missing-platform', 'attestation-descriptor',
            ...
            'private-field-filter', 'unsupported-spdx')) { 2 } else { 0 }
    ...
    if ($Scenario -eq 'aggregate-observed') {
        ...
        Assert-True ($LASTEXITCODE -eq 0) 'Status failed on serialized identity.'
