...
- **Strongly-consistent (CP):** a Raft layer behind the `IRaftConsensus` seam — `RaftSharedKeyValueStore` (linearizable `CompareAndSwapAsync` + `WatchAsync`) and `RaftLeaderElection` (native single-leader election). The DI default is a single-node [`RaftCs`](https://github.com/marcschier/raft-cs) replica via `RaftCsConsensus`; `InProcessRaftConsensus` is a lighter deterministic in-process backend, and a multi-node `RaftNode` (NanoMsg transport + file WAL) plugs in for multi-pod clusters.
