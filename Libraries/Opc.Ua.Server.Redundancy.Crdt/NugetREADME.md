...
`OPCFoundation.NetStandard.Opc.Ua.Server.Redundancy.Crdt` adds **active/active (multi-writer)** replication to `OPCFoundation.NetStandard.Opc.Ua.Server.Redundancy`, built on conflict-free replicated data types (the [`Crdt`](https://www.nuget.org/packages/Crdt) and [`Crdt.Transport`](https://www.nuget.org/packages/Crdt.Transport) packages).
