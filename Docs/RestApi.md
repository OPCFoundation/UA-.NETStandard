...
`OPCFoundation.NetStandard.Opc.Ua.Bindings.Rest` and complements (does
...
- **Client side**: symmetric `IRestApiClient` in
  `OPCFoundation.NetStandard.Opc.Ua.Client` under
  `Libraries/Opc.Ua.Client/Rest/`.
...
services
    ...
    .AddRestApiTransport()      // adds REST routes alongside them
    .AddRestApiAnonymousAuth(); // or AddRestApiBearerAuth(...) etc.
```
...
`RestApiServiceRoutes` in `Stack/Opc.Ua.Core/Stack/RestApi/`.
...
- `TransportProfileUri = http://opcfoundation.org/UA-Profile/Transport/https-restapi`
...
`JsonDecoder` driven by `RestApiBodyCodec`. The encoder shape matches
the spec's component schemas property-for-property; see
`RestApiEncoderConformanceTests` in
`Tests/Opc.Ua.Core.Tests/Stack/RestApi/`.
...
services.AddOpcUa()
    .AddHttpsTransport()
    .AddRestApiTransport()
    .AddRestApiAnonymousAuth()                                      // baseline
    .AddRestApiBearerAuth(opt => { opt.Authority = "..."; })        // JWT
    .AddRestApiBasicAuth(async (user, password) => { /* ... */ })   // RFC 7617
    .AddRestApiMutualTlsAuth(opt => { opt.AllowedCertificateTypes = ... });
```
...
`RestApiInvocationContext.Identity` and flows into the
...
```csharp
services.AddRestApiTransport(opt =>
{
    opt.HostingMode = RestApiHostingMode.SharedWithHttpsListener; // default
    opt.DefaultEncoding = RestApiEncoding.Compact;                // spec default
});
```
...
On the client, set `RestApiClientOptions.RequestTimeout =
...
using var client = RestApiClient.Create(
    new Uri("https://server:4843/"),
    new RestApiClientOptions { Encoding = RestApiEncoding.Compact });
...
`Libraries/Opc.Ua.Client/Rest/`. It is multi-TFM (net48 /
...
`IRestApiClient` exposes one strongly-typed method per service plus a
...
- **TFM constraint.** `Opc.Ua.Bindings.Rest` targets `net8.0`,
...
- **Own-listener mode** (deferred) — `RestApiTransportOptions.UseOwnListener`
