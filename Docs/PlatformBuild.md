...
The following Nuget packages are released in a monthly cadence, unless security issues require hot fixes.
...
[OPCFoundation.NetStandard.Opc.Ua.PubSub](https://www.nuget.org/packages/OPCFoundation.NetStandard.Opc.Ua.PubSub/)
...
1. .NET Standard 2.0 (supported by Opc.Ua.Types assembly only)
2. .NET Standard 2.1
...
To reduce the ci build overhead and the number of tests to be run in Visual Studio, only (4.) and (6.) are part of a qualifying ci build to pass a pull request.
All other platforms are only tested in weekly scheduled or manual ci builds.

By default, unit tests for (1.) and (2.) can not be run out of the box. In order to test the other platforms in a command line window or in Visual Studio, you can define a custom environment variable to target a specific build. E.g. to target a .NETStandard2.1 build, the test executables are compiled with .NET but the libraries target netstandard2.1, to force the use of that target.
