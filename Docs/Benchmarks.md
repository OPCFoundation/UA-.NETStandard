...
(*) These figures are hardware- and load-dependent and are directional; they improve on dedicated hardware where the client and server run on separate machines. The 500–1500 rows are the original in-process baseline; the 2000–10000 rows were measured in a dedicated quiet-machine A/B campaign with `ServerConfiguration.DecoupleHeldPublishRequests` on (the default) and `MaxRequestThreadCount` sized to the active establishment concurrency (~200), **not** to the session count. Under that configuration this 6-core machine cleanly established every session and delivered 100 % of notifications up to **~4000** concurrent sessions - a marked improvement over the earlier ~1500–2000 ceiling - because a parked `Publish` no longer pins a request-processing worker (the S1 decoupling), which frees workers for the CPU-bound establishment (`CreateSession`/`ActivateSession`) burst. Sizing the pool to the session count instead (e.g. 10500) was ~2× slower to establish and reached a *lower* ceiling (thread oversubscription during the handshake burst). At 10000 the machine still tipped into a secure-channel connect storm (repeated handshake aborts/retries and tens of thousands of channel attempts for the target) and did not establish the full set within the connect budget - so on this hardware establishment, not notification delivery, remains the ceiling; more cores push it higher. When sessions establish but do not receive notifications the last column instead reads `No (N drops)`, where `N` is the number of sessions that did not receive notifications within the window. See [Server Session Scalability](ServerScalability.md) for the code-referenced analysis and the built-in controls for degrading gracefully under load.
