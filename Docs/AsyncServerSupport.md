...
Overflow and discard — the FIFO/LIFO `DiscardOldest` behaviour and the `Overflow` status bit — remain a property of each monitored item's own queue. The shared per-node channel is a lossless conduit and does not drop notifications, so a monitored item's configured queue policy is always applied to the full stream of changes.
