...
`Applications/RedundantClient/docker-compose.yml` runs the client and server in one compose file where **both scale independently** (`--scale server=N --scale client=M`) and **both log their failover / HA behavior, including data loss**: the client turns transparent reconnects into explicit `FAILOVER:`, `DATA LOSS:`, and `HA OK: Counter continued …` lines, and the server logs its active/standby role transitions and a per-replica `Counter` heartbeat. The default (active/active, eventual) shows data loss on failover because each replica owns an independent `Counter`; the `Strong/docker-compose.yml` variant (active/passive Raft) keeps the `Counter` on a linearizable store and shows **no** data loss. See the [RedundantClient README](../Applications/RedundantClient/README.md#run-with-docker-compose-scale-the-client-and-server-independently-see-failover--data-loss).
