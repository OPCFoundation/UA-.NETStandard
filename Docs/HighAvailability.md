...
`Server.ServerRedundancy` is exposed as the **generated subtype instance** that matches the configured mode — `TransparentRedundancyType` for `Transparent`, `NonTransparentRedundancyType` for `Cold`/`Warm`/`Hot`/`HotAndMirrored`, or the base `ServerRedundancyType` for `None`. The mode-specific members (`CurrentServerId`, `ServerUriArray`) therefore come from the generated information model (`TransparentRedundancyState` / `NonTransparentRedundancyState`) rather than hand-built `PropertyState` nodes, so a client that browses the node sees the correct `HasTypeDefinition` and only the members the standard defines for that subtype. The base node's well-known NodeIds (`Server_ServerRedundancy_RedundancySupport`, `_RedundantServerArray`, `_CurrentServerId`, `_ServerUriArray`) are preserved when the node is promoted to the subtype.

The mode can also be changed at runtime through the injectable `IServerRedundancyController.ChangeModeAsync(mode)`: the controller re-materializes `Server.ServerRedundancy` as the new subtype in place and raises a `ModelChange` so connected clients observe the new type definition and members. The generic building block underneath is `IDiagnosticsNodeManager.ReplacePredefinedInstanceSubtypeAsync(...)`, which swaps a predefined instance node for a differently-typed instance while preserving its identity, well-known child NodeIds and values.
