...
* Source generated [NodeManagers](SourceGeneratedNodeManagers.md) - Emit an `AsyncCustomNodeManager` from a model design XML and wire callbacks via the fluent `INodeManagerBuilder` API; supports NativeAOT single-file servers (samples: [MinimalBoilerServer](../Applications/MinimalBoilerServer), [MinimalPumpServer](../Applications/MinimalPumpServer)). Covers engineering units, property initialisation, alarms, simulation timers, instance creation, NAMUR-style supervision and multi-model composition extensions.
* [Companion specification libraries](CompanionSpecLibraries.md) - Guide for packaging a companion specification as a reusable model + server + client library trio. Worked example: `Opc.Ua.Di` / `Opc.Ua.Di.Server` / `Opc.Ua.Di.Client`. References WoT-Con and GDS as additional patterns.
* [Device Integration (DI) developer guide](Opc.Ua.Di/README.md) - End-to-end documentation for the DI library trio: fluent [DeviceBuilder](Opc.Ua.Di/DeviceBuilder.md), [hosting integration](Opc.Ua.Di/Hosting.md) (`AddOpcUaDi` / `ConfigureDevicesFor`), [LockService](Opc.Ua.Di/LockService.md), [SoftwareUpdate](Opc.Ua.Di/SoftwareUpdate.md) package store, and [client helpers](Opc.Ua.Di/ClientHelpers.md) (`DiLockClient`, `DiTopologyClient`, `SoftwareUpdateClient`).
