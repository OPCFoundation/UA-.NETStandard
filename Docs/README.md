...
* [State Machines](StateMachines.md) - Generic, extensible Part 16 state-machine API on the source-generated `*TypeClient` proxies (`GetCurrentFiniteStateAsync`, `ObserveFiniteTransitionsAsync`, `WaitForStateAsync`) and a fluent server-side builder (`StateMachineBuilder`, `FluentFiniteStateMachineState`). Vendor state machines inherit the API automatically.
