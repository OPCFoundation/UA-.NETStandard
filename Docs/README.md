...
* [Aggregates (Part 13)](Aggregates.md) - All 37 standard Part 13 v1.05.06 aggregate functions over historical data: server `AggregateManager` / calculators, native push-down vs framework fallback, `AnnotationCount` via the annotation provider, `AggregateConfiguration` defaults, and the client `ReadProcessedAsync` helper.
