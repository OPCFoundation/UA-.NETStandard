...
* [High Availability and Distributed Address Space](HighAvailability.md) - Opt-in sharing of address-space state (topology + values) and session state across replicas for active/passive and active/active redundancy: the `Opc.Ua.Server.Distributed` building blocks (`ISharedKeyValueStore`, `INodeStateStore`, `IAddressSpaceSynchronizer`, `ILeaderElection`, `IDistributedValueCache`, `IServiceLevelProvider`, `ISharedSessionStore`), exposing `ServiceLevel` / `Server.ServerRedundancy` to clients, and Kubernetes replicaset deployment guidance.
  * [High Availability — Security &amp; Threat Model](HighAvailabilitySecurity.md) - STRIDE threat model, trust boundaries, and the store-hardening mitigations (authenticated encryption, key rotation, single-use nonce, secure session sharing) for the distributed-HA feature.
  * [Kubernetes Deployment for High Availability](KubernetesDeployment.md) - Worked replicaset deployment: StatefulSet + headless `Service`, leader election, readiness tied to `ServiceLevel`, KEK / shared-certificate provisioning via Secrets.
