...
* Source generated [NodeManagers](SourceGeneratedNodeManagers.md) - Emit a `CustomNodeManager2` from a model design XML and wire callbacks via the fluent `INodeManagerBuilder` API; supports NativeAOT single-file servers (sample: [ConsoleBoilerServer](../Applications/ConsoleBoilerServer)).
