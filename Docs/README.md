...
* [Subscription Triggering (V2)](SubscriptionTriggering.md) - Declarative + imperative SetTriggering API for the V2 subscription engine. N:M triggering relationships via `MonitoredItemOptions.TriggeredByNames` and `ISubscription.SetTriggeringAsync(IMonitoredItem, ...)`; batched apply pipeline; per-link `SetTriggeringResult`; snapshot round-trip; automatic replay on recreate/reconnect.
