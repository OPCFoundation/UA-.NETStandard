...
Notes: only UADP encoding is defined for the Ethernet mapping (no JSON over `opc.eth://`); there is no transport-level security (unlike `opc.dtls://`) — use the message-level PubSub security (`SecurityMode` / SecurityGroups) as for UDP; frames exceeding `MaxFrameSize` (the link MTU) cannot be sent, so enable UADP chunking or raise the MTU; the native AF_PACKET / BPF backends are exercised by opt-in / manual tests only (they need privileges and real hardware), while CI uses the in-memory loopback backend.
