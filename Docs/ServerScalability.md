...
> The reference server applies **admission control and rate limiting** on by default with conservative limits — see [Rate Limiting and Admission Control](RateLimiting.md). The listener socket backlog is configurable (default 512); a connection rate limiter at accept and a bounded session-establishment concurrency limiter shed excess load with a fast `BadServerTooBusy` instead of aborting mid-handshake; and the CreateSession server-nonce RSA signing runs outside the establishment lock. The same admission is available on the HTTPS/Kestrel transport through `AddHttpsRateLimiter`, and a cooperating client honors the backpressure with a server-signal-aware adaptive backoff, a client-wide connect admission gate, and a best-effort retry-after hint (`RetryAfterMs=N`) carried in the `BadServerTooBusy` fault. Carrying that hint in a transport response header, so it is honored precisely without requiring client diagnostics, is a possible future refinement.
