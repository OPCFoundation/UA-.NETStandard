...
A common assumption is "one blocked thread per held `Publish`". That is **not** what happens: when no subscription is ready, `SessionPublishQueue.PublishAsync` returns a `TaskCompletionSource`-backed task (`Libraries/Opc.Ua.Server/Subscription/SessionPublishQueue.cs`) and the worker `await`s it — the OS thread is released, not spinning.

> **Addressed.** A parked long-poll no longer occupies a worker slot. A per-request park sink (`IRequestParkSink`, carried on `RequestLifetime`) is raised at the single park site — `SessionPublishQueue.PublishAsync` queuing the incomplete `TaskCompletionSource` — and `ServerBase.RequestQueue` awaits **park-or-complete** (`Task.WhenAny`), releasing the active-worker slot at the park point instead of at completion. The response is still delivered out-of-band via the request's value-task source. Only `Publish` requests carry a sink, so the non-`Publish` hot path is unchanged. A small worker pool can now hold many thousands of outstanding Publishes, so `MaxRequestThreadCount` no longer has to scale with session count. The behavior can be reverted with `ServerConfiguration.DecoupleHeldPublishRequests = false`.

Historically, the request-queue **accounting** coupled the worker budget to the number of concurrently-held Publishes: in `ServerBase.RequestQueue.WorkerLoopAsync` the active-worker counter was incremented for the *entire* duration of the awaited request, including the whole long-poll wait; `ScheduleIncomingRequest` spawns additional worker loops up to `MaxRequestThreadCount` whenever the active count reaches the current worker count, and the queue constructor raises the process-wide `ThreadPool` maximum to `MaxRequestThreadCount`.

The practical consequence *before this fix*: to serve N sessions you had to size `MaxRequestThreadCount ≥ N` and the process `ThreadPool` ceiling ballooned accordingly (the load test sets 10500 for a 10000-session case).
...
5. **Decouple held Publishes from the worker/thread accounting** so a parked long-poll does not consume a worker slot and the `ThreadPool` ceiling does not scale with session count. Addresses S1. **(Delivered.)**
...
- **Option C — Steady-state scaling (well beyond a few thousand).** Decouple held Publishes from the worker-count accounting (S1 — **delivered**, `ServerConfiguration.DecoupleHeldPublishRequests`, default on) so a small worker pool holds many thousands of long-polls, and make the diagnostics update incremental. Needed to go far past the establishment ceiling once A and B land.
...
- **`MaxRequestThreadCount`** caps concurrent request processing. With `DecoupleHeldPublishRequests` on (the default), a held `Publish` releases its worker while it waits (S1), so `MaxRequestThreadCount` no longer has to scale with session count — size it for the *active* (non-parked) request concurrency. If you set `DecoupleHeldPublishRequests = false`, the legacy coupling returns and a server serving N sessions again needs `MaxRequestThreadCount` well above N or held Publishes starve other services with `BadRequestTimeout`. **`MinRequestThreadCount`** pre-warms the pool so a connect burst is not throttled by thread-pool cold-start.
- **`DecoupleHeldPublishRequests`** (default `true`) releases a held `Publish`'s worker at the park point so a small worker pool can hold many thousands of outstanding long-polls; set `false` to restore the legacy inline behavior.
