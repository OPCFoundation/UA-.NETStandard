...
| `CertificateTypesProvider` (class) | `ICertificateRegistry` (composed in `ICertificateManager`). Use `using CertificateEntry? e = manager.AcquireInstanceCertificate(securityPolicyUri);` (caller-owned — dispose the entry) and `manager.LoadCertificateChainAsync(...)`. |
