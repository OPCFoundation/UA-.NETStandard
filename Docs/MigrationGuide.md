...
- [Migration Guide](#migration-guide)
  - [Migrating from 1.5.378 to 1.6.x](#migrating-from-15378-to-16x)
    ...
    - [Session State Persistence](#session-state-persistence)
      - [`SessionStateEncoder` removed](#sessionstateencoder-removed)
      ...
      - [Binary encoding format is not backward compatible](#binary-encoding-format-is-not-backward-compatible)
...
**Breaking Change**: The session and subscription state persistence model has been redesigned. The `SessionStateEncoder` helper class has been removed — `SessionState`, `SubscriptionState`, and related types now implement `IEncodeable` directly via the `[DataType]` source generator.

#### `SessionStateEncoder` removed

Previously, saving and restoring session state required the `SessionStateEncoder` helper that manually read/wrote each field. This class no longer exists. Instead, `SessionState`, `SessionOptions`, `SubscriptionState`, and `MonitoredItemState` are annotated with `[DataType]` and use the standard `Encode`/`Decode` methods generated by the source generator.

To register the session state types with the encodeable factory:
...
#### Binary encoding format is not backward compatible

The binary encoding format for session state has changed. Existing persisted session state files (e.g., `.opcua` files saved with the previous `SessionStateEncoder`) **cannot** be loaded by the new `SessionConfiguration.Create()` method. Delete old persisted session files and re-establish sessions after upgrading.
