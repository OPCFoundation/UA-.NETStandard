...
- **Facade failover integration test.** `Tests/Opc.Ua.Sessions.Tests/RedundantClientSessionFailoverIntegrationTests.cs` drives two `RedundantClientSession` replicas (a controllable leader election) against a live `ReferenceServer`: a follower's async call blocks and sync members throw `BadInvalidState`; after a forced handoff the promoted replica's **same facade reference** keeps serving browse/read over the swapped-in session. Passes on net10 and net48.
- **Dynamic compose replica scaling (gossip).** `Applications/RedundantServer` gained a DNS peer-discovery mode (`HA_PEER_DISCOVERY=dns` + `HA_SERVICE_NAME`, self-exclusion + startup retry) and a `docker-compose.scale.yml` single-service file so `docker compose up --scale server=N` self-configures gossip peers. Active/active eventual only; dynamic Raft scaling remains the Kubernetes StatefulSet path (`UseKubernetesRaftConsensus`).
- **Warm/Hot client standby.** `Applications/RedundantClient --standby <Cold|Warm|Hot>` selects the replica-set standby mode, and `--suite` with `--replicas > 1` runs the workload through the promoted leader's `RedundantClientSession`.
