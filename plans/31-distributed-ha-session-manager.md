...
- **S6 `f-e2e-test-secure` — remaining.** Two in-process servers sharing one store + a real `ManagedSession` client exercising the full failover reconnect (signature validation against the restored, single-use nonce) and the negative security cases. The security-decision logic is already unit-tested; this adds the full-path/network validation.
