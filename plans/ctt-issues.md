...
* Alarm `Test_004.js` now reaches 25 configured InputNodes instead of 14. Every primary failure is the same client-side `ReadHelper` `BadInvalidState` emitted from inside the alarm callback, followed by `Unable to read input node`; the increase confirms the corrected CTT project mappings exercise more valid alarm types. It is not a server Read result.
* Alarm `Test_002.js` is unchanged at 28 errors and remains the branch-unaware Retain oracle described above.
* Enable `Test_002.js` is unchanged at 45 errors. The log shows all 15 conditions transition to `EnabledState=Disabled`, but the script still calls the three-argument `AddMessage` function with four arguments for each EnabledState, Retain, Event Time, or TransitionTime mismatch. The fourth `reason` argument is discarded, so the result cannot identify which comparison failed. The live type- and instance-method regression validates EnabledState, Retain, TransitionTime, disabled-event suppression, and re-evaluation on enable.
* Confirm `Test_001.js` successfully confirms every tested condition and validates ConfirmedState and Comment. Its only failures are the Discrete, OffNormal, and SystemOffNormal summaries after `ValidateRetain` rejects `Retain=true` while the current event is inactive, acknowledged, and confirmed. The log does not inspect outstanding branches. Part 9 §5.5.2 explicitly states that the Retain bit on the current Event is True while any ConditionBranches require Operator input, and the server regression reproduces that exact prior-active-branch state.
