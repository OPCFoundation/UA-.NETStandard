# Filtered Missing Nodes — Truly Missing After Source Generator Fixes

**Date:** 2026-04-25 20:59:04
**Server:** `opc.tcp://localhost:48010/Quickstarts/ReferenceServer`
**Auth:** Admin session (sysadmin/demo, SecurityMode=None)

## Executive Summary

| Metric | Value |
|--------|-------|
| Spec nodes (ns=0) | 5,283 |
| Server nodes (ns=0, admin) | 6,006 |
| Total missing | 1,233 |
| Filtered (Optional/Placeholder/SKS) | 439 |
| **Truly missing** | **794** |

### Filter Criteria Applied

1. **Optional children** (ModellingRule=Optional): direct or any ancestor
2. **Placeholder nodes** (ModellingRule=MandatoryPlaceholder or OptionalPlaceholder)
3. **SecurityKeyServerAdmin role** nodes (SKS/PubSub admin methods)
4. **ExposesItsArray** modelling rule nodes

## Breakdown of Truly Missing

| Category | Count | % of Total |
|----------|-------|-----------|
| encoding_nodes | 384 | 48.4% |
| role_instances | 172 | 21.7% |
| other_missing | 84 | 10.6% |
| trust_list_and_cert | 36 | 4.5% |
| pubsub_and_sks | 31 | 3.9% |
| session_and_server_diag | 30 | 3.8% |
| orphaned_type_children | 25 | 3.1% |
| user_management | 10 | 1.3% |
| server_config_and_certs | 8 | 1.0% |
| filesystem | 5 | 0.6% |
| ha_aggregate_config | 5 | 0.6% |
| program_state | 4 | 0.5% |
| **Total** | **794** | **100%** |
...
Default Binary/XML/JSON encoding objects for DataTypes (no parent in XML)

**NodeClass:** Object=384
...
WellKnownRole object children (Identities, Applications, methods)

**NodeClass:** Variable=114, Method=58
...
Other missing nodes not fitting above categories

**NodeClass:** Variable=79, Method=4, Object=1
...
### Trust List And Cert (36)

TrustList objects and TrustListType children

**NodeClass:** Method=24, Variable=11, ObjectType=1
...
PubSub, SecurityKeyServer types and instances

**NodeClass:** Variable=20, Method=7, ObjectType=4
...
### Session And Server Diag (30)

ServerDiagnostics and SessionDiagnostics variables
...
### Orphaned Type Children (25)

Type-level properties with no parent ref (e.g. NodeVersion, Icon)

**NodeClass:** Variable=24, Object=1
...
UserManagement type and instance nodes

**NodeClass:** Method=8, Variable=2
...
### Server Config And Certs (8)

ServerConfigurationType and CertificateGroupType children

**NodeClass:** Method=4, Object=2, ObjectType=1, Variable=1
...
FileSystem object and methods

**NodeClass:** Method=4, Object=1
...
### Ha Aggregate Config (5)

HA AggregateConfiguration properties

**NodeClass:** Variable=4, Object=1
...
ProgramStateMachineType optional properties

**NodeClass:** Variable=4
...
### What the Source Generator Fixes Addressed

| Fix | Impact | Status |
|-----|--------|--------|
| Encoding nodes emitted | Reduces encoding_nodes category | Partial — 384 remain (orphaned in spec) |
| EventNotifier auto-inferred | Correct attribute values | ✅ Working (not measurable by node count) |
| Type definition RBAC wrapped | No false access-denied | ✅ Working |
| MR=None children on types | Emit concrete instances | Partial — role instances still missing |

### Remaining Work

The 794 truly missing nodes (admin) break down as:

1. **Encoding nodes (384):** These have no parent reference in Opc.Ua.NodeSet2.xml.
   They are DataType encoding objects that need to be emitted by the predefined
   nodes source generator as top-level objects.
2. **Role instances (172):** WellKnownRole children
   (Identities, Applications, methods like AddIdentity, RemoveIdentity, etc.).
   These are MR=None children of the role objects under Server/ServerCapabilities/RoleSet.
3. **Trust list/cert (36):** TrustListType
   children and CertificateGroup TrustList instances.
4. **Session/server diagnostics (30):**
   ServerDiagnosticsSummary sub-variables and session diagnostics arrays.
5. **PubSub/SKS (31):** PubSubKeyPushTargetType
   and SecurityGroupType children.
6. **Other (84):** Various other nodes.
