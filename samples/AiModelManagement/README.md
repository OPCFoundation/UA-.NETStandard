...
| | |
|---|---|
| `Model/` | The companion specification's NodeSet, source-generated at build time |
| `AiModelManagement.Bridge/` | Reaching a model: the backend abstraction, the OpenAI-compatible client, credential resolution |
| `AiModelManagementServer/` | The Server. Publishes the address space and serves the specification's Methods |
| `AiModelManagementClient/` | A console client that browses from the entry point and exercises what it finds |
| `deploy/helm/` | The chart, its tests, and a cluster smoke test |
| `../../tests/AiModelManagement.Tests/` | Unit and integration tests against a fake backend |
...
```powershell
python samples/AiModelManagement/verify_backend.py 5273
```

Then, in two more terminals:

```powershell
dotnet run --project samples/AiModelManagement/AiModelManagementServer
dotnet run --project samples/AiModelManagement/AiModelManagementClient
```
...
helm install ai samples/AiModelManagement/deploy/helm/ai-model-management `
...
```powershell
helm lint samples/AiModelManagement/deploy/helm/ai-model-management
python samples/AiModelManagement/deploy/helm/chart_tests.py

# slow, needs Docker and kind; creates and deletes a cluster
pwsh samples/AiModelManagement/deploy/helm/smoke-test.ps1
```
