...
Run the matching client workflow against a secured endpoint with provisioned
certificate trust. For an isolated lab, the trust-only opt-in is:

```bash
dotnet ConsoleReferenceClient.dll --historian --autoaccept \
