...
dotnet run --project samples\Redundancy\RedundantServer\RedundantServer.csproj -- --port 62544
```
...
docker compose --env-file samples\Redundancy\RedundantServer\active-active-eventual.env `
  -f samples\Redundancy\RedundantServer\docker-compose.yml up --build

# Active/active, strong consistency (3-node RaftCs, shared linearizable store).
docker compose --env-file samples\Redundancy\RedundantServer\active-active-strong.env `
  -f samples\Redundancy\RedundantServer\docker-compose.yml up --build

# Active/passive, strong consistency (3-node RaftCs; one active writer, two hot standbys).
docker compose --env-file samples\Redundancy\RedundantServer\active-passive.env `
  -f samples\Redundancy\RedundantServer\docker-compose.yml up --build

# GetEndpoints load direction over the Raft cluster (sets HA_BALANCING_URL on every replica).
docker compose -f samples\Redundancy\RedundantServer\LoadDirection\docker-compose.yml up --build
...
docker compose -f samples\Redundancy\RedundantServer\Transparent\docker-compose.yml up --build
```
...
```powershell
docker compose -f samples\Redundancy\RedundantServer\Scale\docker-compose.yml up --build --scale server=5
```
...
```powershell
docker compose -f samples\Redundancy\RedundantServer\Scale\docker-compose.yml --profile clients up --build --scale server=3 --scale client=2
```
...
docker compose --env-file samples\Redundancy\RedundantServer\active-passive.env `
  -f samples\Redundancy\RedundantServer\docker-compose.yml --profile clients up --build

# 2. In another shell, stop the active/leader replica and watch the client keep going:
docker compose -f samples\Redundancy\RedundantServer\docker-compose.yml stop server-a
```
