...
`RedundantPubSub` is a runnable OPC UA PubSub high-availability sample that demonstrates an active/standby publisher set and subscriber-side SequenceNumber continuity across publisher failover.
...
`ROLE=subscriber` or `--role subscriber` starts a UDP/UADP subscriber plus a raw UADP sequence monitor. The regular subscriber sink logs decoded fields, and the monitor logs SequenceNumber continuity lines.
...
From the repository root, the default `.env` selects the `strong` profile: three named publisher replicas with Raft leader election and a Raft-backed shared store, plus one subscriber.

```powershell
docker compose -f Applications/RedundantPubSub/docker-compose.yml up --build
```

Trigger failover by stopping the current active publisher container. The publisher logs include `Publisher <owner>: <component> -> Active` or `Standby`; stop the active one from another terminal:
