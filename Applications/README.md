...
It uses the OPC Foundation UA .NET Standard Library. Therefore it supports both the opc.tcp and https transports. There is a .NET framework based server with UI and a .NET Core console version of the server which runs on any OS supporting [.NET Standard 2.1](https://docs.microsoft.com/dotnet/articles/standard) or .NET 8.0 and later.
...
The **ConsoleReferenceServer** has also a command line option `-ctt` which starts the Alarms when the server is started. The following Alarm instances are available for monitoring:
- `AlarmConditionType`
