...
UA0029  | Migration | Warning  | Replace the `SecurityPolicies` lookup and cryptography statics (`GetUri`, `GetDisplayName`, `GetDisplayNames`, `IsValidSecurityPolicyUri`, `GetDefaultUris`, `GetDefaultEccUris`, `GetDefaultDeprecatedUris`, `Encrypt`, `Decrypt`) with `ISecurityPolicyRegistry` members, or `SecurityPolicies.Default` where no container is in scope. The `Encrypt`/`Decrypt` `ILogger` argument is gone. Policy URI constants are unaffected. See docs/MigrationGuide.md#ua0029.
UA0030  | Migration | Warning  | Remove use of the server `ISubscription` publish-pipeline members (`PublishTimerExpired`, `Acknowledge`, `PublishTimeout`, `SubscriptionTransferred`, `AvailableSequenceNumbersForRetransmission`, `QueueOverflowHandler`, `SessionClosed`, `Publish` — internalized; `ItemReadyToPublish`/`ItemNotificationsAvailable` — deleted no-ops) and of `SessionPublishQueue` (internal): the publish pipeline is server-internal in 2.0, and custom subscriptions derive from `Subscription`. See docs/MigrationGuide.md#ua0030.
UA0031  | Migration | Warning  | Replace `ISubscriptionManager.Republish`/`SetTriggering`/`ModifyMonitoredItemsAsync`/`SetMonitoringModeAsync` with `TryGetSubscription` (throw `Bad_SubscriptionIdInvalid` on miss) followed by the operation on `ISubscription`. See docs/MigrationGuide.md#ua0031.
