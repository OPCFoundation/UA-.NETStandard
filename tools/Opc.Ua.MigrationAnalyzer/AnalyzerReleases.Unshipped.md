...
UA0024  | Migration | Warning  | Replace `IServerInternal`/`ISession`/`ISubscription` `DiagnosticsLock`/`DiagnosticsWriteLock` access with `UpdateServerDiagnostics(...)` / `UpdateDiagnostics(...)` (and `ReadDiagnostics` for projections).
