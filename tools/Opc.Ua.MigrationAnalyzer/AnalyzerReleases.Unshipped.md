...
UA0024  | Migration | Warning  | Replace the `SecurityPolicies` lookup and cryptography statics (`GetUri`, `GetDisplayName`, `GetDisplayNames`, `IsValidSecurityPolicyUri`, `GetDefaultUris`, `GetDefaultEccUris`, `GetDefaultDeprecatedUris`, `Encrypt`, `Decrypt`) with `ISecurityPolicyRegistry` members, or `SecurityPolicyRegistry.Default` where no container is in scope. The `Encrypt`/`Decrypt` `ILogger` argument is gone. Policy URI constants are unaffected. See docs/MigrationGuide.md#ua0024.
