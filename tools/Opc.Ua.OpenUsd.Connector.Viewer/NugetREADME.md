...
- `UsdViewOptions.PrimPicked` is supported by renderer-backed pointer picking when the live renderer exposes a public
  or safely discoverable pick backend. `UsdViewOptions.PickMode` defaults to that path and automatically falls back to
  the command prim; the fallback watches `/World/IntentCommand` by default and raises the callback when its
  `targetPrim` relationship or attribute changes.
