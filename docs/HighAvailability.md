...
## Server redundancy (as per Part 4 §6.6.2)
...
Standardized redundancy nodes and methods (`Add*`):

| API | Description |
| --- | --- |
| `AddServerRedundancy(options)` | Publishes the `Server.ServerRedundancy` metadata (RedundancySupport, `ServerUriArray` / `RedundantServerArray`, `NTRS` capability). Metadata only — it does not drive `ServiceLevel`. |
| `AddServerServiceLevel(provider)` | Drives the `Server.ServiceLevel` node from an `IServiceLevelProvider` (for example `LeaderServiceLevelProvider`; use a `ConstantServiceLevelProvider` for a fixed level). |
| `AddRequestServerStateChange(options)` | Enables the standard `Server.RequestServerStateChange` method for administrator-driven Maintenance / NoData failover. |

Beyond-spec distributed building blocks (`Use*`):
...
## ServiceLevel and load balancing (as per Part 4 §6.6.2.4.2 and §6.6.2.4.3)
...
## Manual failover and Maintenance (as per Part 4 §6.6.5)
...
- `UseDistributedAddressSpace(...)` mirrors node topology and values through `INodeStateStore`, helping each server present the same NodeIds and browse paths. Each node manager is restricted to its declared non-standard `NamespaceUris`; namespace-zero infrastructure such as `ServerDiagnostics` and `ServerRedundancy` remains replica-local, and a replicated root may not contain descendants outside the same ownership partition. A node manager that uses a custom partition and therefore returns `null` for `NamespaceUris` must also implement `ILocalAddressSpaceOwnership`, providing a stable replica-set `PartitionId` and an `OwnsNode(NodeId)` predicate. Hydration updates an existing node graph in place, preserving runtime callbacks, handles, and generated node types while applying the shared state authoritatively. A protected partition marker distinguishes a completed empty partition from an interrupted initial seed, so restart cleanup cannot resurrect deleted roots or delete roots omitted by a partial bootstrap. Startup registers and buffers the live feed before hydration, closing the scan-to-subscription lost-update window. Startup and failover promotion use a snapshot + delta-log fast path when the store implements `INodeStateSnapshotStore` (the default `InMemoryNodeStateStore` does): the writer periodically publishes a chunked, atomically-swapped snapshot of the graph and trims a bounded delta log, so a standby reads a handful of large chunks and replays only the changes after the snapshot instead of transferring one key/value entry per node. Missing or invalid snapshot chunks make the snapshot non-authoritative and trigger complete streamed hydration. Each record carries a single-writer monotonic sequence, and the standby applies the snapshot, delta log, buffered live feed, and ongoing changes through a per-key sequence guard so the paths are idempotent and cannot apply a stale change over a newer one. When no snapshot has been published (or the store lacks the capability) the standby falls back to two streamed passes — `EnumerateAsync` for topology and `EnumerateValuesAsync` for values.
...
The consensus engine is pluggable through the `IRaftConsensus` seam. By default the DI registration uses a single-node in-package Raft replica (`DefaultRaftConsensus.CreateSingleNode()`, a real RaftNode with in-memory storage/transport that elects itself leader); `InProcessRaftConsensus` is a lighter deterministic in-process alternative for tests and in-process replica sets. For a multi-pod cluster, build a multi-node replica with `DefaultRaftConsensus.CreateCluster(nodeId, memberIds, transport, …)` — a `RaftCs.Transport.NanoMsg` transport (peers addressed by DNS) plus an in-memory or durable `RaftCs.Storage.File` write-ahead log (WAL) — and register it through `RaftConsensusFactory`. On Kubernetes, `UseKubernetesRaftConsensus` (in `Opc.Ua.Redundancy.Kubernetes`) does this for you from the StatefulSet ordinal and headless-Service DNS. RaftCs ships its own `IRaftTransport` (NanoMsg), so Raft shares the NanoMsg substrate with the CRDT gossip layer. The `samples/Redundancy/RedundantServer` sample (`docker-compose.yml` with the `active-passive.env` env file) is a runnable multi-node example.
...
Because the value cache writes through the shared `INodeStateStore`, its confidentiality/integrity is governed by the same `IRecordProtector` as the rest of the mirrored state (fail-closed for external stores without a protector). Cross-process value continuity therefore requires a store that propagates writes between replicas: it is verified on the strong-consistency (Raft) topology; on active/active eventual it depends on the CRDT address-space gossip transport. The `samples/Redundancy/RedundantServer` sample wires its `HighAvailability.Counter` through this provider — see the sample README's *Sharing values across replicas* section.
