...
`CreateInstanceOfOpenUsdRepresentationType` and a manually assigned reference type. Use its
`assignNodeId: false` overload only when the enclosing runtime-created subtree assigns NodeIds later. The sample
