...
| `Session` → `ManagedSession`, V2 subscription engine, GDS-client `Task` → `ValueTask` modernisation, removed obsolete GDS APIs, durable subscriptions, PubSub, reverse-connect, server `ISubscription` publish-pipeline members / `SessionPublishQueue` internalized (analyzer `UA0030`), `ISubscriptionManager` routing wrappers collapsed (analyzer `UA0031`) | [`sessions-subscriptions.md`](sessions-subscriptions.md) |
