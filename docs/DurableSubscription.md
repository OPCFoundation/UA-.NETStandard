...
- The Quickstarts durable-subscription store writes format version 3. Version 1 records remain readable with publishing enabled and no persisted owner application URI; the retired version 2 is rejected. Current records persist publishing state and the owning client ApplicationUri so an anonymous durable subscription can be transferred after restart. User-name passwords are removed before persistence, and issued-token subscriptions are not persisted because their bearer token is the identity.
