...
The aggregation server contains no Pump-specific generated code and does not reference the DI, Machinery, or Pumps server/model assemblies. The complete DI/Machinery/Pumps/Pump instance shape is runtime-loaded from the files in [`samples/WotAggregation/Documents`](../samples/WotAggregation/Documents) through the generic WoT-to-NodeSet converter, runtime NodeSet loader, and target-mapping binding runtime.
...
dotnet run --project samples\WotAggregationClient\WotAggregationClient.csproj -f net10.0 -- `
  ...
  --documentsDirectory .\samples\WotAggregation\Documents
```
...
## Checked-in document set and order

[`documents.json`](../samples/WotAggregation/Documents/documents.json) declares the dependency order:
...
The client validates that every dependency exists, topologically orders the manifest, and fails before upload when the manifest contains a missing or cyclic dependency. `WotRegistryClient.LoadDocumentsAsync` also guarantees that Thing Models are processed before Thing Descriptions while preserving the caller's relative order within each document kind.
...
The sample aggregation server sets `AutoRefresh = false` so the four uploads do not cause four intermediate projections. The explicit final `Refresh` activates one complete dependency closure.
...
* [Developing WoT protocol bindings](WoTBindingDevelopment.md)
