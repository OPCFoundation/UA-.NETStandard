...
- **Mixed framework builds.** With `CustomTestTarget=netstandard2.1`, libraries target .NET Standard while test consumers target net8.0. Gate tests for modern-only library interfaces on the selected library feature, not `NET8_0_OR_GREATER` alone. The xRegistry projects use `XREGISTRY_HTTP_MODERN` only when `NetStandardTests` is false; portable HTTP-client, native and synchronization tests still run in the .NET Standard leg.
