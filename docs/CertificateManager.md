...
- **Push commits invalidate the validator.** After the `ApplyChanges` response boundary, and before the §7.10.9 sweep re-validates any channel, `ConfigurationNodeManager` calls `ICertificateTrustListManager.NotifyTrustListChanged` for every committed scope. The certificate manager drops its cached validation cores — including the `UseValidatedCertificates` fast path, which additionally never skips the revocation check any more — so the sweep is guaranteed to observe the committed stores rather than a cached snapshot.
- **Out-of-band changes trigger the same sweep.** The server subscribes to the certificate manager's `CertificateChanges` stream: any `TrustListUpdated` or `CrlUpdated` event — raised by `WriteTrustListAsync`, an `ITrustListTransaction` commit, or an HA shared-store update — is coalesced (a burst of events folds into one pass per scope union) and dispatched through the same `IPushConfigurationTrustListEffectHandler`. Notifications the manager raises about its own `ApplyChanges` commits are skipped, because the deferred apply already runs the precise committed-scope sweep. Two origins need explicit host action: a **legacy non-transactional `TrustList`** raises no event unless the host opts in by calling `TrustList.SetTrustListChangeNotifier(manager, scope)` after constructing it (nothing in the stack wires this automatically), and a bare file copied into a store directory raises no event at all and is only picked up by the next validation (bounded by the directory store's 30-second cache age for new channels).
...
Three further collaborators are internal to the stack and not replaceable: the static `PushCertificateValidation` helpers (certificate/issuer-chain and TrustList checks, subject and key-size rules, endpoint-reference checks), the `NamespaceMetadataRegistry` behind the Part 5 `NamespaceMetadata` members of `IConfigurationNodeManager`, and the `CertificateAlarmScheduler` that refreshes and evaluates the §7.8.3 alarms. `ConfigurationNodeManager` itself is a partial class split into one `ConfigurationNodeManager.*.cs` file per concern (Push methods, certificate slots, ApplyChanges, trust material, certificate alarms, namespace metadata).
