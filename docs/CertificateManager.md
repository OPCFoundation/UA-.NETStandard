...
- **Out-of-band changes trigger the same sweep.** The server subscribes to the certificate manager's `CertificateChanges` stream: any `TrustListUpdated` or `CrlUpdated` event — raised by `WriteTrustListAsync`, an `ITrustListTransaction` commit, an HA shared-store update, or a legacy non-transactional `TrustList` wired via `TrustList.SetTrustListChangeNotifier` — is coalesced (a burst of events folds into one pass per scope union) and dispatched through the same `IPushConfigurationTrustListEffectHandler`. Notifications the manager raises about its own `ApplyChanges` commits are skipped, because the deferred apply already runs the precise committed-scope sweep. A bare file copied into a store directory raises no event and is therefore still only picked up by the next validation (bounded by the directory store's 30-second cache age for new channels).
