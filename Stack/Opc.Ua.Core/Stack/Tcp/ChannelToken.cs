...
namespace Opc.Ua.Bindings
{
    ...
    public sealed class ChannelToken : IDisposable
    {
        ...
        private static readonly byte[] s_channelSecretLabel = System.Text.Encoding.UTF8.GetBytes("opcua-channelsecret");

        /// <summary>
        /// Creates the SecureChannelSecret after the channel was initially established.
        /// </summary>
        public void CreateSecureChannelSecret()
        {
            if (Secret != null)
            {
                var salt = Utils.Append(s_channelSecretLabel, ClientNonce, ServerNonce);

                switch (SecurityPolicy.KeyDerivationAlgorithm)
                {
                    case KeyDerivationAlgorithm.HKDFSha384:
                    {
                        using var hmac = new HMACSHA384(salt);
                        SecureChannelSecret = hmac.ComputeHash(Secret);
                        break;
                    }
