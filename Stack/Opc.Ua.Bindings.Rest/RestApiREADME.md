...
`OPCFoundation.NetStandard.Opc.Ua.Bindings.Rest` is the
...
services
    ...
    .AddRestApiTransport();     // adds the REST controllers + discovery URI
```

The binding emits a dedicated `TransportProfileUri`
(`http://opcfoundation.org/UA-Profile/Transport/https-restapi`) in
...
See the [REST binding guide](https://github.com/OPCFoundation/UA-.NETStandard/blob/master/Docs/RestApi.md)
...
models, hosting modes, and the symmetric C# `IRestApiClient`.
